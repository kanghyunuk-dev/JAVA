package Ch01;
// 고객이 사과장수 에게 사과 1개를 구매한다
// 클래스 : 고객, 사과장수
// 기능 : 구매 (구매는 고객이 한다 -> 고객 클래스에 생성)

// 클래스	: 고객 (Buyer)
// 속성(기능을 수행할때 필요요소) : 보유금액(myMoney : int), 사과개수(appleCnt : int)
// 기능	: 구매하기(사과장수 에게 보유금액의 일부) - pay(Seller seller , int money) : void)


// 클래스	: 사과장수(Seller)
// 속성 	: 보유금액(myMoney : int), 사과개수(appleCnt : int), 사과가격(price : int)
// 기능 	: 판매하기(구매자의 돈) : 사과개수 - sales(int money) : int
...
class Buyer {
	...
	public void pay(Seller seller, int money) {
		// 내 돈에서 구매의사 금액만큼 차감(내보유금액>구매의사금액)
		// 판매자에게 구매의사금액 전달
		// 판매자는 구매자에게 사과 개수 전달
		int receivedAppCnt;
		if(myMoney<money) {
...
class Seller {
	...
	public int sales(int money) {
		...
		// 판매자의 보유 사과개수 에서 차감(보유사과개수 vs 구매자 요청사과개수)
		// appleCnt -= calAppCnt;
		appleCnt -= calAppCnt;
		// 계산된 사과개수 반환
		return calAppCnt;
