...
// 호출 할 때 마다 자기 안의 숫자를 1 증가시켜 반환 (가변 클로저)
interface TickBox {
	int tick();
}
...
public class C06ClosureBasic {
	...
	private static TickBox makeTicker(int start) {
		int[] n = {start};		// 람다가 캠처해서 들고 다닌 "주머니"
		return () -> n[0]++;	// 현재 값 반환 후 +1
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("(2) box2.open() = " + box2.open()); // 박스 안 : 바나나
		System.out.println("(3) box3.open() = " + box3.open()); // 박스 안 : 포도
		
		// 2) 배열 트릭 사용
//		int n = 0;
//		Task addOne = ()->{n++;}; // 람다 내부에 외부변수 사용시 final
		int n [] = {0};
		Task addOne = ()->{n[0]++;}; // 배열요소로 참조하는 값은 변경가능
		addOne.run();
		addOne.run();
		addOne.run();
		System.out.println("n[0] : " + n[0]);
		
		// 3) 인스턴스 변수 / static 변수 사용
